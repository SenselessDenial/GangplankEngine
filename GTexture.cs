...
namespace GangplankEngine
{
    public class GTexture : IDisposable
    {
        ...
        {
            Texture = Calc.TextureFromFile(file);
            Texture.Name = file;
        ...
        public void DrawOutline(Vector2 pos, Color color, Color outlineColor)
        {

            for (int i = -1; i <= 1; i++)
                for (int j = -1; j <= 1; j++)
                {
                    Draw(pos + new Vector2(i, j), outlineColor);
                }
        ...
        {
            DrawOutline(pos, color, Color.Black);
        }
