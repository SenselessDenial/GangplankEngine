...
namespace GangplankEngine
{
    public class PixelFont
    {
        ...
        public PixelFont(GTexture texture, string xmlFile) : this()
        {
            ...
            XmlDocument doc = Calc.XmlFromString(Path.Combine(Engine.ResourceFolderPath, xmlFile));
        ...
        public int MeasureY(char value)
        {
            PixelFontCharacter temp;
            if (TryGetValue(value, out temp))
            {
                return temp.Texture.Height;
            }
            return 0;
        }
        ...
        public int MeasureY(string value)
        {
            return MeasureY(value[0]);
        }
        ...
        public void DrawOutline(string value, Vector2 position, Color color, Color outlineColor, Alignment alignment = Alignment.Left)
        {
            Vector2 currentPos;
            int size = MeasureX(value);
            switch (alignment)
            ...
            for (var i = 0; i < value.Length; i++)
            {
                ...
                if (toDraw != null)
                {
                    toDraw.Texture.DrawOutline(new Vector2(currentPos.X - toDraw.XOffset, currentPos.Y - toDraw.YOffset), color, outlineColor);
                    currentPos.X += toDraw.XAdvance;
                }
            }
        }
        ...
        {
            DrawOutline(value, position, color, Color.Black);
        }
